...
export default function TodayAgenda({ eventsByDate }) {
  ...
  return (
    <div className="sprout-surface p-6 flex flex-col max-h-[300px]">
      ...
      {events.length === 0 ? (
        <div className="mt-6 space-y-2 text-center text-amber-700">
          <p className="text-lg font-medium">You&apos;re free today</p>
          <p className="text-sm">Enjoy the day or plan something.</p>
        </div>
      ) : (
        <div className="flex-1 overflow-y-auto space-y-3">
          {events.map((e) => {
            ...
            return (
              <div key={e.id} className={`sprout-card p-3 ${colorClass}`}>
